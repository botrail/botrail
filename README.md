...
- **Robots from URDF, Xacro, or USD** — including Isaac Sim articulations
  ...
  Mimic joints (URDF `<mimic>`, USD `NewtonMimicAPI` / `PhysxMimicJointAPI`)
  are followed, so a two-finger gripper costs one DOF, not two. Multiple
  robots per cell, with tick-checked inter-robot collisions and zone
  interlocks.
