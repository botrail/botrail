...
impl ProjectFile {
    pub fn from_json(json: &str) -> Result<Self, ProjectError> {
        ...
        match probe.version {
            ...
            2 => {
                // `robots: []` is a working cell too — a conveyor line, an
                // AGV loop — so an empty list loads instead of erroring.
                serde_json::from_str(json).map_err(|e| ProjectError::Json(e.to_string()))
            }
...
impl Scene {
    ...
    pub fn apply_project(&mut self, project: &ProjectFile) -> Result<(), ProjectError> {
        ...
        for (spec, enabled, visible, walkable, legend, physics) in obstacles {
            ...
            if walkable {
                self.set_obstacle_walkable(&final_name, true)
                    .map_err(|e| ProjectError::Scene(e.to_string()))?;
            }
        }
        for (i, robot_msg) in project.robots.iter().enumerate() {
...
pub fn generate_python(project: &ProjectFile) -> String {
    ...
    for (i, robot_msg) in project.robots.iter().enumerate() {
        ...
        // its tools' rows (`arm/tool`), zones watching it, I/O nodes — and
        // a script that let the model's own name stand in would break at
        // the first of them.
        if let Some(name) = &robot_msg.name {
    ...
    if project.robots.is_empty() {
        // A cell of devices and obstacles alone — the scene opens empty.
        out.push_str("scene = bt.Scene()\n");
    }
    ...
    for o in &project.obstacles {
        ...
        if o.walkable {
            out.push_str(&format!(
                "scene.set_obstacle_walkable({:?}, True)\n",
                o.name
            ));
        }
        ...
        match &device.kind {
            ...
            crate::wire::DeviceKindMsg::LinearAxis {
                ...
                let stops = if stops.is_empty() {
                    ...
                    let entries: Vec<String> =
                        stops.iter().map(|s| format!("{:?}: {}", s.name, s.at)).collect();
                    format!(", stops={{{}}}", entries.join(", "))
                };
                out.push_str(&format!(
                    "scene.add_linear_axis({:?}, objects=[{}], axis={}, speed={speed}, range={}, position={position}{stops})\n",
                    device.name,
                    items.join(", "),
                    py_tuple(axis),
                    py_tuple(range),
            ...
            crate::wire::DeviceKindMsg::Vehicle {
                ...
                let waypoints: Vec<String> = path
                    ...
                    .iter()
                    .map(|p| {
                        // A level waypoint round-trips in the 2-tuple form
                        // the author wrote.
                        if p[2].abs() < 1e-12 {
                            format!("({}, {})", p[0], p[1])
                        } else {
                ...
                let mut extras = String::new();
                if path.ring {
                    extras.push_str(", ring=True");
                }
                if *allow_reverse {
                    extras.push_str(", allow_reverse=True");
                }
                ...
                if let Some(a) = aerial {
                    extras.push_str(&format!(
                        ", drive=\"aerial\", climb_speed={}, descent_speed={}",
                        a.climb_speed, a.descent_speed
                    ));
                    if let Some(psi) = a.fixed_yaw {
                        extras.push_str(&format!(", fixed_yaw={psi}"));
                    }
                }
                out.push_str(&format!(
                    "scene.add_vehicle({:?}, body=[{}], path=[{}], stations={{{}}}, \
                     speed={speed}, turn_speed={turn_speed}, start={start:?}{extras}{deck})\n",
                    device.name,
                    members.join(", "),
                    waypoints.join(", "),
                    stations.join(", "),
            ...
            crate::wire::DeviceKindMsg::Lift {
                ...
                let members: Vec<String> = car.iter().map(|n| format!("{n:?}")).collect();
                let stop_items: Vec<String> = stops
                    .iter()
                    .map(|s| format!("{:?}: {}", s.name, s.at))
                    .collect();
                // The stored zone sits at the reference position; the
                // authoring call gives it where the car stands at `start`.
                let start_at = stops
                    .iter()
                    .find(|s| &s.name == start)
                    .map(|s| s.at)
                    .unwrap_or(0.0);
                let zone_at_start = [
                    zone_pose.position[0] + axis[0] * start_at,
                    zone_pose.position[1] + axis[1] * start_at,
                    zone_pose.position[2] + axis[2] * start_at,
                ];
                let quat = if zone_pose.quaternion == [0.0, 0.0, 0.0, 1.0] {
                    String::new()
                } else {
                    format!(", zone_quaternion={}", py_tuple(&zone_pose.quaternion))
                };
                out.push_str(&format!(
                    "scene.add_lift({:?}, car=[{}], zone_position={}, zone_size={}, \
                     stops={{{}}}, speed={speed}, axis={}{quat}, start={start:?})\n",
                    device.name,
                    members.join(", "),
                    py_tuple(&zone_at_start),
                    py_tuple(zone_size),
                    stop_items.join(", "),
                    py_tuple(axis),
                ));
...
fn py_action(action: &ActionMsg) -> String {
    ...
    match action {
        ...
        ActionMsg::Device { device, command } => match command {
            ...
            crate::wire::DeviceCommandMsg::MoveToStop { stop } => {
...
mod tests {
    ...
    #[test]
    fn empty_robot_lists_load_as_a_robotless_scene() {
        // `robots: []` is a working cell (a conveyor line, an AGV loop) —
        // it loads, applies, and regenerates as `bt.Scene()`. Built from an
        // actually robot-less scene: one with robot-addressed leftovers is
        // a hand-edit, and the generator merely degrades on those.
        let mut scene = Scene::empty();
        scene
            .add_obstacle(
                "bench",
                ...
                Isometry3::translation(1.0, 0.0, 0.25),
            )
            .unwrap();
        let json = scene.to_project().to_json();
        ...
        assert_eq!(rebuilt.obstacles().len(), 1);
        let code = generate_python(&back);
        assert!(code.contains("scene = bt.Scene()\n"), "{code}");
        assert!(code.contains("scene.add_box(\"bench\""), "{code}");
    }
    ...
    fn generated_python_contains_the_full_recipe() {
        ...
        // Identity base stays out of the generated constructor; the
        // instance name stays in.
