...
| URDF/Xacro | xurdf | 自作クレートのため拡張が容易。package:// のメッシュパス解決は botrail 要件に合わせて確認済み。**mimic joint は上流(xurdf 0.5)に実装して botrail から使う**という当初の想定どおりの動線が成立した(A3) |
...
- **A3(mimic joint)[完了 2026-08-03]** — 実機のグリッパ記述は `<mimic>` が前提(Robotiq 2F、Franka Hand 等)なのに、botrail は 2 本指を独立 DOF として読んでいた。q の並びが実機と食い違い、スライダも 2 本出て、指を片方だけ動かせてしまう状態だった。**上流(xurdf 0.5.3)に `<mimic>` パースを入れ、botrail 側は「mimic joint は DOF を持たない」というモデル不変条件として通した**(§3 のエコシステム方針どおりの動線)。(1) `Joint.mimic = { source_joint, multiplier, offset }`。`from_parts` で **mimic チェーンを平坦化**(`a = m1·(m2·q + o2) + o1`)し、サイクル・固定ジョイント参照・未知参照は `ModelError` で拒否。q_index は mimic には振らないので `dof()` が自然に減る。(2) FK は `joint_value()` 経由。**Jacobian は mimic に列を与えず、源ジョイントの列へ multiplier 倍で加算する**(連鎖律)— これを落とすと IK が「指が動く分」を勘定に入れられない。有限差分一致テストあり。(3) `driving_joints()` は mimic 先祖を*それを駆動するジョイント*に読み替える(コンベアトラッキングの contested joint 判定が指リンクで壊れないように)。(4) **リミットは源へ逆写像しない**判断: MoveIt 式に mimic 側 limit を源へ写して交差させると、`multiplier="-1"` なのに limit を源からコピーしただけの URDF(実在する)で可動域が 1 点に潰れて実ロボットが動かなくなる。幾何+運動学に徹する botrail では mimic の limit は情報として持つだけにし、値は常に関係式から決める。(5) **USD**: 書き出しは `PhysicsJointStateAPI` の timeSamples を mimic joint にも派生値で書く(リンク姿勢は元から FK 焼き込みなので usdview では動いていたが、joint state を信じる読み手にはグリッパが消えていた)。取り込みは `PhysxMimicJointAPI` — PhysX の拘束式は **`qA + G·qB + γ = 0`** なので `multiplier = -G·k_A/k_B`、`offset = -γ·k_A`(k は角度なら deg→rad、並進なら metersPerUnit)。ジョイント自身の軸以外の instance に付いていた場合は表現できないので警告して落とす。(6) **wire/studio**: `JointMsg.mimic` を追加。**USD ロボットはクライアント側 FK なので、これが無いと指だけ動かない**(URDF ロボットはサーバ FK のリンク姿勢経由なので元から動く)。JointPanel は mimic joint を「follows <源>」の読み出し専用行で出す。(7) Python は `Robot.mimic_joints` / `Robot.joint_values(q)`。**非互換の注意**: mimic を持つロボットは DOF が減るので、旧版で保存した `.botrail`(指を独立 DOF として保存したもの)は `apply_project` の DOF 一致チェックで弾かれる — 直し方は自明(教示値を取り直す)であり、そもそも旧版の値は片指だけ動く誤った状態だったので互換を残さない。
