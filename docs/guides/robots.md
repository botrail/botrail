...
a revision is the same machine re-cut from a better source, so short names
follow it forward instead of breaking. Different *products* stay ambiguous
and raise, listing what matched. Name a revision outright to pin it, and pin
the revision a cell was taught on: a newer cut can move the geometry, the
joint zero or the TCP.
...
acquisition.
Downloads land in the standard Hugging Face cache. A package whose meshes
cannot be redistributed is `recipe_only`: it never wins a short name, and
naming it in full raises with a pointer to building it locally with
botrail-catalog-builder. A kit always loads the exact component revisions
it was assembled from. The resolved id and dataset commit stay visible in
the robot's source and on its BOM line.

A package directory you built yourself loads, manifest included, with:
...
`from_package` makes no network request and keeps the manifest's product ID,
frames, order and mounting declarations (it needs PyYAML, included in the
catalog extra). Its revision is `local-sha256:<digest>` of the files inside
the package, and a saved `.botrail` project bundles the geometry, so the
project and its generated Python reload without the directory. Loading just
the URDF or USD carries none of the package identity.

The Robotiq demos run on the public r2 reference hand and its ES-062 kit —
Menagerie's BSD-2-Clause meshes on botrail's joint tree and teaching frames,
...
robot = arm.attach_tool(kit)   # includes the ES-062 coupling once, under `kit_`
robot.dof        # 6 + 1
robot.tcp_link   # 'kit_gripper/tcp' — the gripper's declared TCP, so IK targets the grasp center
```
