...
`<stem>_report.md|.json` last, with the digests of everything written
before it. `--sequence` scopes every program-dependent document as well as
the bake. `--manifest` keeps `<stem>_manifest.json` — the file hashes and
the conditions the bake ran under.

`DIR` is created when missing; a set already there is overwritten file by
file and anything else in the directory is left alone, so the command can be
re-run in place. The files are generated from one isolated cell snapshot in
a temporary directory and moved over only after every exporter succeeds.
Besides the `simulate` bake options, accepts `--dt` (scan interval, default
0.01 s) and `--plan-resolution` (planner stride, default 0.05). Bakes are
kinematic. A program that cannot compile to the robot dialect (a 7-axis arm,
an uncovered branch) is a warning on stderr, not an export failure.
