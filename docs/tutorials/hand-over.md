...
```python
--8<-- "examples/engineering/cell_deliverables_demo.py:38:71"
```
...
are three BOM lines with article numbers and masses. The UR control box is
placed by [`bt.parts.controller`](../guides/standard-parts.md): the box on
the sheet, the `UR` I/O node the cycle already wired (adopted, wiring kept)
and its BOM line — with the robot cable length on it — are one thing, and
without it the bill would still carry a
[`simple_arm/controller` line](../guides/parts-and-bom.md#the-bill-of-materials)
nobody had placed. And every `set_part` names a product without changing
the cycle at all — identity is a separate layer from geometry and
behaviour.

## Writing the set

```python
--8<-- "examples/engineering/cell_deliverables_demo.py:85:120"
```
