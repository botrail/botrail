...
| `welding/` | `spot_gun_mounting_demo.py` — BX250L + NIMAK mounting and arm motion, with the gun fixed closed. `weld_station_demo.py` — one spot-weld station. `weld_line_demo.py` — the four-station body-in-white line. `line_balance_sweep.py` — move a spot between stations and watch the takt. |
